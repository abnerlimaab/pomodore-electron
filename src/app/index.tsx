...
export default function App() {
  ...
  const { colorScheme, palette, railExpanded } = useUiSettingsStore();
  const { setGrupos } = useTemaStore();
  const { setAtividades } = useAtividadeStore();
  ...
  useEffect(() => {
    ...
    }).catch((e: unknown) => console.error('Failed to load initial data:', e));
  }, []);
  ...
  useEffect(() => {
    ...
    ipc.db.getAtividades({})
      ...
      .catch((e: unknown) => console.error('Failed to refresh atividades:', e));
  }, [currentPage]);
