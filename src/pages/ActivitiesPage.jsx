...
export default function ActivitiesPage() {
  const { grupos, atividades, setGrupos, setAtividades } = useAppStore();

  const [selectedGrupo, setSelectedGrupo] = useState(null);
  const [grupoDialog, setGrupoDialog] = useState({ open: false, editing: null });
  const [atividadeDialog, setAtividadeDialog] = useState({ open: false, editing: null });
  ...
  const [atividadeForm, setAtividadeForm] = useState({ nome: '', tema_id: '', status: 'ativa' });
  ...
  const loadAtividades = async () => {
    ...
    if (!data?.error) setAtividades(data || []);
  };
  ...
  const handleSaveGrupo = async () => {
    ...
    const duplicate = grupos.find(
      g => g.nome.toLowerCase() === nome.toLowerCase() && g.id !== editingId
    );
    if (duplicate) {
      setGrupoNomeError('Já existe um grupo com este nome.');
      return;
    }
    setGrupoNomeError('');
  ...
  const handleDeleteGrupo = async () => {
    ...
    await api.deleteTema(deleteDialog.item.id);
    setDeleteDialog({ open: false, type: null, item: null });
    if (selectedGrupo?.id === deleteDialog.item.id) setSelectedGrupo(null);
    await loadGrupos();
  ...
  const openEditAtividade = (atividade) => {
    setAtividadeForm({
      nome: atividade.nome,
      tema_id: atividade.tema_id || '',
  ...
  const handleSaveAtividade = async () => {
    ...
    const duplicate = atividades.find(
      a => a.nome.toLowerCase() === nome.toLowerCase() && a.id !== editingId
    );
    if (duplicate) {
      setAtividadeNomeError('Já existe uma atividade com este nome.');
      return;
    }
    setAtividadeNomeError('');
    const payload = { ...atividadeForm, nome, tema_id: atividadeForm.tema_id || null };
    if (editingId) {
      await api.updateAtividade({ id: editingId, ...payload });
    } else {
  ...
  const handleDeleteAtividade = async () => {
    if (!api || !deleteDialog.item) return;
    await api.deleteAtividade(deleteDialog.item.id);
    setDeleteDialog({ open: false, type: null, item: null });
    await loadAtividades();
    setSnackbar({ open: true, message: 'Atividade excluída!', severity: 'info' });
  };

  const handleToggleStatus = async (atividade) => {
    if (!api) return;
    const newStatus = atividade.status === 'ativa' ? 'inativa' : 'ativa';
    await api.updateAtividade({ ...atividade, status: newStatus });
    await loadAtividades();
  };
          ...
          <ListItem disablePadding>
            ...
            >
              <ListItemText
                ...
                primaryTypographyProps={{ variant: 'body2', fontWeight: selectedGrupo === null ? 600 : 400 }}
              />
              <Chip
                label={atividades.length}
                size="small"
              ...
              >
                <Box
                  sx={{
                    ...
                    height: 12,
                    borderRadius: '50%',
                    backgroundColor: grupo.cor_hex || '#6750A4',
                    mr: 1,
                    flexShrink: 0,
                  }}
                />
                <ListItemText
                  ...
                  primaryTypographyProps={{
                    variant: 'body2',
                    fontWeight: selectedGrupo?.id === grupo.id ? 600 : 400,
                    noWrap: true,
                  }}
                />
                <Chip
                  label={atividades.filter(a => a.tema_id === grupo.id).length}
                  size="small"
                ...
                <Box sx={{ display: 'flex', gap: 0 }}>
                  ...
                  <IconButton
                    ...
                    onClick={(e) => {
                      e.stopPropagation();
                      setDeleteDialog({ open: true, type: 'grupo', item: grupo });
                    }}
        ...
        <List sx={{ overflow: 'auto', flex: 1, py: 1 }}>
          {filteredAtividades.length === 0 && (
            <Box sx={{ p: 4, textAlign: 'center' }}>
              <Typography variant="body2" color="text.secondary">
                Nenhuma atividade. Clique em "Adicionar" para criar.
              </Typography>
            </Box>
          )}
          ...
          {filteredAtividades.map((atividade) => (
            <ListItem
              key={atividade.id}
              sx={{
                opacity: atividade.status === 'inativa' ? 0.5 : 1,
            ...
            >
              {atividade.tema_cor && (
                <Box
                  sx={{
                    ...
                    height: 8,
                    borderRadius: '50%',
                    backgroundColor: atividade.tema_cor,
                    mr: 1.5,
                    flexShrink: 0,
                  }}
                />
              ...
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
                ...
                <IconButton
                  size="small"
                  onClick={() => setDeleteDialog({ open: true, type: 'atividade', item: atividade })}
                  sx={{ p: 0.5, color: 'error.main' }}
                >
                  <DeleteIcon sx={{ fontSize: 16 }} />
                </IconButton>
      ...
      <Dialog open={grupoDialog.open} onClose={() => setGrupoDialog({ open: false, editing: null })} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          <TextField
            ...
            onChange={(e) => { setGrupoNomeError(''); setGrupoForm(f => ({ ...f, nome: e.target.value })); }}
            fullWidth
            size="small"
            sx={{ mt: 1, mb: 2 }}
            error={!!grupoNomeError}
            helperText={grupoNomeError}
          />
          <Typography variant="body2" color="text.secondary" gutterBottom>
            Cor
          </Typography>
          <ColorPicker
            value={grupoForm.cor_hex}
            onChange={(c) => setGrupoForm(f => ({ ...f, cor_hex: c }))}
          />
        </DialogContent>
      ...
      <Dialog open={atividadeDialog.open} onClose={() => setAtividadeDialog({ open: false, editing: null })} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          <TextField
            ...
            onChange={(e) => { setAtividadeNomeError(''); setAtividadeForm(f => ({ ...f, nome: e.target.value })); }}
            fullWidth
            size="small"
            sx={{ mt: 1, mb: 2 }}
            error={!!atividadeNomeError}
            helperText={atividadeNomeError}
          />
          <FormControl fullWidth size="small" sx={{ mb: 2 }}>
            <InputLabel>Grupo (opcional)</InputLabel>
            <Select
              value={atividadeForm.tema_id || ''}
              onChange={(e) => setAtividadeForm(f => ({ ...f, tema_id: e.target.value }))}
          ...
          <FormControl fullWidth size="small">
            ...
            <Select
              ...
              onChange={(e) => setAtividadeForm(f => ({ ...f, status: e.target.value }))}
              label="Status"
            >
      ...
      <Dialog open={deleteDialog.open} onClose={() => setDeleteDialog({ open: false, type: null, item: null })} maxWidth="xs" fullWidth>
        ...
        <DialogContent>
          <Typography variant="body2">
            Tem certeza que deseja excluir "{deleteDialog.item?.nome}"?
        ...
        <DialogActions sx={{ px: 3, pb: 3 }}>
          ...
          <Button
            onClick={deleteDialog.type === 'grupo' ? handleDeleteGrupo : handleDeleteAtividade}
            variant="contained"
            color="error"
          >
            Excluir
