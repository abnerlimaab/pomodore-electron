...
function GroupedTimeline({ sessions }) {
  if (!sessions.length) {
    return (
  ...
  const sorted = [...sessions].sort((a, b) => new Date(b.inicio) - new Date(a.inicio));
      ...
      {groups.map(({ label, sessions: daySessions }) => (
        <Box key={label} sx={{ mb: 3 }}>
          <Typography
            ...
            fontWeight={700}
            sx={{ display: 'block', mb: 1, textTransform: 'uppercase', letterSpacing: '0.1em', fontSize: '0.7rem' }}
          >
            {label}
          </Typography>
          {daySessions.map(session => {
            ...
            const primaryActs = session.atividades?.filter(a => a.prioridade === 'Primaria') || [];
            const secondaryActs = session.atividades?.filter(a => a.prioridade === 'Secundaria') || [];
            ...
            return (
              ...
              >
                ...
                <Box sx={{ width: Math.max(8, Math.min(200, (duration / 3600) * 200)), minWidth: 8, mt: 0.5 }}>
                  <Box sx={{ height: 10, backgroundColor: color, borderRadius: 1, mb: 0.5, opacity: 0.9 }} />
                  {secondaryActs.length > 0 && (
                    <Box sx={{ height: 6, backgroundColor: color, borderRadius: 1, opacity: 0.4 }} />
                  )}
                </Box>
                <Box sx={{ flex: 1 }}>
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5, flexWrap: 'wrap', mb: 0.25 }}>
                    <Chip
                      label={session.tipo}
                      size="small"
                      sx={{ height: 18, fontSize: '0.65rem', backgroundColor: `${color}33`, color, fontWeight: 600 }}
                    />
                    {primaryActs.map((a, i) => (
...
export default function ReportsPage() {
  ...
  // Groups extracted from loaded sessions
  const groups = useMemo(() => {
    const map = new Map();
    sessions.forEach(s => s.atividades?.forEach(a => {
  ...
  // Sessions with valid duration, filtered by search/group
  const filteredSessions = useMemo(() => {
    ...
    return valid.filter(s => {
      ...
      const matchesSearch = !searchQuery ||
        acts.some(a => a.nome?.toLowerCase().includes(searchQuery.toLowerCase()));
      const matchesGroup = !selectedGroup ||
        acts.some(a => String(a.tema_id) === String(selectedGroup));
  ...
  const { barData, donutData, totalTime, totalSessions } = useMemo(() => {
    ...
    filteredSessions.forEach(session => {
      total++;
      totalSeconds += session.duracao_total_segundos;
      session.atividades?.forEach(act => {
        ...
        grupoMap[key].minutes += Math.floor(session.duracao_total_segundos / 60);
      });
            ...
            {isEmpty ? (
              <Box sx={{ textAlign: 'center', py: 6 }}>
                <Typography variant="body1" color="text.secondary">
                  {isFiltering
                    ? 'Nenhuma sessão encontrada para o filtro aplicado.'
                    : 'Nenhuma sessão registrada neste período.'}
                </Typography>
                {!isFiltering && (
                  <Typography variant="caption" color="text.secondary">
                    Conclua algumas sessões para ver suas estatísticas aqui.
                  </Typography>
                )}
              </Box>
            ) : (
                  ...
                  <Paper sx={{ p: 2, flex: '1 1 200px', minWidth: 180, border: '1px solid', borderColor: 'divider' }}>
                    ...
                    <ResponsiveContainer width="100%" height={220}>
                      <PieChart>
                        ...
                        <Tooltip
                          formatter={(value) => [formatSeconds(value * 60), 'Tempo']}
                          contentStyle={{
