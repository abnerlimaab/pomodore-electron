...
import { Box, Popover, Typography, Tooltip, Divider } from '@mui/material';
import PaletteIcon from '@mui/icons-material/Palette';
import CheckIcon from '@mui/icons-material/Check';
import { PALETTES } from '../config/theme';
import { useUiSettingsStore } from '../model/uiSettingsStore';
...
export default function ThemeMenu({ expanded = false }: ThemeMenuProps) {
  const { palette, setPalette, colorScheme } = useUiSettingsStore();
  ...
  const trigger = (
    <Box
      ...
      sx={{
        ...
        px: expanded ? 1.5 : 0, py: 1, borderRadius: 2, cursor: 'pointer',
        width: expanded ? '100%' : 'auto', color: 'text.secondary',
        '&:hover': { bgcolor: 'action.hover' },
      }}
    >
      <PaletteIcon fontSize="small" sx={{ flexShrink: 0 }} />
      {expanded && <Typography variant="body2" noWrap sx={{ fontWeight: 400 }}>Aparência</Typography>}
    </Box>
  );

  return (
    <>
      {expanded ? trigger : <Tooltip title="Aparência" placement="right">{trigger}</Tooltip>}
      ...
      <Popover
        ...
        slotProps={{ paper: { sx: { ml: 1, p: 2, borderRadius: 3, width: 200, border: '1px solid', borderColor: 'divider' } } }}
      >
          ...
          {Object.entries(PALETTES).map(([key, p]) => {
            ...
            return (
              <Tooltip key={key} title={p.label} placement="top">
                ...
                >
                  {selected && <CheckIcon sx={{ fontSize: 16, color: '#fff', filter: 'drop-shadow(0 1px 1px rgba(0,0,0,0.5))' }} />}
                </Box>
              </Tooltip>
            );
          })}
        ...
        <Box sx={{ display: 'flex', gap: 1, mt: 1 }}>
          {[{ id: 'light', label: 'Claro' }, { id: 'dark', label: 'Escuro' }].map(({ id, label }) => (
            <Box
              key={id}
              onClick={() => useUiSettingsStore.getState().setColorScheme(id as 'light' | 'dark')}
              sx={{
                flex: 1, py: 0.75, borderRadius: 2, textAlign: 'center', cursor: 'pointer',
                fontSize: '0.75rem', fontWeight: colorScheme === id ? 600 : 400,
