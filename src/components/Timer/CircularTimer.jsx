...
function useTimerSize() {
  ...
  useEffect(() => {
    const update = () => {
      // available width = window - nav rail (60px) - horizontal padding (~80px)
      const available = window.innerWidth - 60 - 80;
      // available height - mode chips (~60px) - controls (~70px) - chips (~80px) - padding (~80px)
...
export default function CircularTimer({ timeLeft, totalSeconds, isRunning, isPaused, isFree, freeElapsed, modeName }) {
  const theme = useTheme();
  const size = useTimerSize();

  let progress = 100;
  if (!isFree && totalSeconds > 0) {
  ...
  const getColor = () => {
    ...
    if (isPaused) return theme.palette.warning?.main || '#F4A261';
    ...
    >
      ...
      >
        <Typography
          ...
          sx={{
            color: 'text.secondary',
            fontSize: size < 180 ? '0.6rem' : '0.75rem',
            fontWeight: 500,
            letterSpacing: '0.1em',
            textTransform: 'uppercase',
            mb: 0.5,
