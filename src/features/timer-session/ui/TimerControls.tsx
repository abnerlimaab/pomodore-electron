...
export default function TimerControls({ isRunning, isPaused, onStart, onPause, onResume, onStop }: TimerControlsProps) {
  ...
  const iconButtonSx = (color?: string): SxProps<Theme> => ({
    ...
    color: '#fff',
    '&:hover': { backgroundColor: color ? theme.palette.error.dark : theme.palette.primary.dark, transform: 'scale(1.05)' },
    ...
    <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mt: 1 }}>
      {(isRunning || isPaused) && (
        <Tooltip title="Interromper">
          <IconButton onClick={onStop} sx={secondaryIconSx}><StopIcon /></IconButton>
        </Tooltip>
      )}
      {!isRunning && !isPaused && (
      ...
      {!isRunning && !isPaused && (
        <Tooltip title="Reiniciar">
          <IconButton onClick={onStop} sx={secondaryIconSx}><ReplayIcon /></IconButton>
        </Tooltip>
      )}
    </Box>
  );
